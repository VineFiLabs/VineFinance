...
async function main() {
    ...
    // const VineHelper = await vineHelper.deploy();
    // const VineHelperAddress = VineHelper.target;
    // console.log("VineHelperAddress:", VineHelperAddress);

    const VineHelperAddress = "0x14BF8bCf166b02bAac77a715821A727cE0d5598e";
    const VineHelper = new ethers.Contract(VineHelperAddress, VineHelperABI.abi, owner);

    const getMarketInfoList = await VineHelper.getMarketInfoList(config.Deployed.Governance, 0n);
    console.log("getMarketInfoList:", getMarketInfoList);

    const getUserSupplyToHookAmount = await VineHelper.getUserSupplyToHookAmount(0, config.Deployed.Governance, owner.address);
    console.log("getUserSupplyToHookAmount:", getUserSupplyToHookAmount);

    const getUserFinallyAmount = await VineHelper.getUserFinallyAmount(0, config.Deployed.Governance, owner.address);
