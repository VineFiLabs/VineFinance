...
library VineLib {
    ...
    function _protocolFeeAmount(
        ...
        if (_finallyTokenAmount > _depositeTotalAmount + 10000) {
            _feeAmount =
                ((_finallyTokenAmount - _depositeTotalAmount) * _feeRate) /
                10000;
        } else if (
            _finallyTokenAmount > _depositeTotalAmount &&
            _finallyTokenAmount <= _depositeTotalAmount + 10000
        ) {
    ...
    function _getUserFinallyAmount(
        ...
        uint256 _finallyTokenAmount,
        uint256 _shareTotalSupply
    ) internal pure returns (uint256 _finallyAmount) {
        if (_finallyTokenAmount == 0) {
            _finallyAmount = 0;
        } else if(_finallyTokenAmount < _depositeTotalAmount){
            _finallyAmount = _userSupplyAmount * _finallyTokenAmount / _depositeTotalAmount;
        }else if (
            _finallyTokenAmount > 0 &&
            _finallyTokenAmount <= _depositeTotalAmount + 10000
        ) {
            _finallyAmount = _userSupplyAmount;
        } else {
            if(_userSupplyAmount > 0 && _userShareAmount == 0){
                _finallyAmount = _userSupplyAmount;
            } else if(_userSupplyAmount >= 0 && _userShareAmount > 0){
                _finallyAmount =
                _userSupplyAmount +
                (((_userShareAmount *
                    (_finallyTokenAmount - _depositeTotalAmount)) / 10000) *
                    (10000 - _curatorFee - _protocolFee)) /
                _shareTotalSupply;
            }else{
                _finallyAmount = 0;
            }
